...
- 布局：桌面为「左档头 + 右正文」两栏，正文与输入区同为 800px 最大宽度，主交互字号从 16px 起。报头下方是「新会话」通栏朱砂主按钮；中间是**会话档案列表**（标题 + 时间/条数，行尾**常驻**半透明 ✕ 删除、悬停变实——早期悬停才出现，用户找不到删除入口，别再改回去）；**「设置」按钮固定在侧栏最底部且不进入滚动区（早期叫「账号与模型」，已随弹窗改版更名）**。窄屏（≤960px）左栏隐藏，顶栏右侧为「会话 · 新会话 · 设置」。侧栏无副标语、无页脚系统信息、无恐龙徽章、无学期读数卡、无导出按钮、无内嵌课表卡。首屏不放日程入口 chip；每次打开默认处于新对话。
...
- **设置弹窗**（`.overlay/.dlg.wide`，2026-09 由整页平铺改版为分栏）：红头标题「设置」；内部是「左目录 + 右内容」——`.set-tabs` 纵向栏目导航（教务账号 / AI 模型 / QQ 机器人 / 待办 / 本地数据，选中态 = 朱砂竖线 + 浮起纸片，与会话列表 `.on` 同一语言；上下方向键移动焦点，空格/回车进入；窄屏 ≤560px 改横排在内容上方，选中态换下划朱砂线）+ `.set-main` 内容区（**只有右侧滚**，左目录钉住；`role=tablist/tab/tabpanel` 语义齐全）。每栏各一段 `.dlg-intro` 说明。「保存设置」只属于凭证类栏目（教务/模型/QQ），切到待办/本地数据两栏时隐藏——这两栏即改即存。教务账号（学号/登录密码，留空不改，只填学号会被拒）；AI 模型（API Key + 型号卡片单选 `.model-cards`，展示当前脱敏摘要与来源、型号；hidden input `#sModel` 载值）；候选清单随 `/api/settings` 的 `models` 字段同步返回（缓存或内置兜底，弹窗绝不因等网络卡住），打开后另发 `GET /api/models` 刷新为该 Key 真实可用的型号，失败退回内置清单并把原因写进说明行。保存走 `POST /api/settings` 的 `model` 字段，清单外型号服务端一律拒绝；切换成功后服务端重建 agent，网页下一条消息即生效（终端界面重启后生效）。文案面向普通使用者，只说明信息加密保存在当前电脑，不暴露实现文件名。关闭方式：✕ / 取消 / 点遮罩 / ESC。保存结果逐行打在 `.setmsg`，全部成功自动关闭。
- **快速提问常驻在输入框上方**（「常用」小标 + chip），不再放侧栏/首屏。**桌面（>960px）chip 换行全量可见**（7 个约两行）——藏进横向滚动用户看不见就不会点；窄屏回单排横滚，避免多行按钮挤压输入区。
- **输入区**：左侧楷体「留言」栏目标签（聚焦变红），输入框聚焦只显示中性深灰细边、不出现朱砂红框；空内容时发送键自动落灰禁用（流式中永远是可点的「停止」）、下方等宽小字快捷键注脚（移动端隐藏）、自适应高度 ≤180px。**Ctrl+V 粘贴文件直接进上传队列**（剪贴板带 files 时接管并 `handleFiles`；纯文本粘贴保持原生行为；演示模式不接）。
- 消息不做气泡：每轮是一行等宽小字题注（`你/助手 · 时间 · 总耗时`）+ 正文。用户消息 = 左侧朱砂竖线 + 纸片底；助手回复 = 通栏 Markdown 排版。**新一轮与时间线子项各有一记 0.15-0.18s 淡入上移**（`turnIn`/`tlIn`；只在节点新建时播一次，流式 Markdown 重渲不重放；历史整表重绘是同帧 append，整页一起淡入；reduced-motion 全局禁用）。
- **工具调用独立建模**：每次调用一张可展开的 `.tool` 卡片（原生 `details/summary`，零 JS 交互）：折叠态一行——行首**恒定的内联 SVG 描线齿轮**（`GEAR` 常量，Lucide settings 路径，`stroke="currentColor"` 取 `--ink-3`；早期用 ▸/✓/✗ 三种字形换状态，用户要求去掉勾叉）、工具名、结果摘要（`.tsum` 弹性位）、**状态文字**（`.tstat`：执行中/完成/失败）、耗时；失败时整框朱砂红边 + 红字。**执行中齿轮匀速旋转**（`.tool.run .tw svg` 的 spin 2.4s，完成/失败自然停转），展开态——`参数` 与 `结果` 两个等宽 pre 块（服务端各截断 1200 字）。SSE 的 end 事件不带配对保证，按 `id` 精确配对为主、同名 FIFO 兜底。非工具错误（网络失败/中断/agent err）仍是一条等宽 `.tline.bad`。
...
- 功能清单：SSE 流式回复（等宽光标 ▌）、**思考过程卡片（流式可见、段末自动折叠）**、停止、失败重试、复制、智能滚动（用户上翻离底即停自动拽底；「回到底部」悬浮按钮已按用户要求于 2026-09-23 删除，别加回来）、**多会话历史（落盘、跨重启、搜索/改名/置顶/删除、窄屏抽屉）**、浏览器附件上传、课表/成绩/考试/通知结构化结果卡、手动刷新变化对比、截止日期待办与 `.ics`、连接检测、本地数据导出/清理、新会话、中文输入法 Enter 保护、textarea 自适应、快捷提问、后台完成标题提醒、移动端适配、`prefers-reduced-motion`。

### 2026-09-22 体验打磨（本次改动）

- **侧栏会话档案按日期归档分组**：`sessionGroup()` 把会话分进 置顶 / 今天 / 昨天 / 7 天内 / 更早，组头 mono 小字 + 计数（与待办分组同一语言），服务端排序保证顺序遍历插组头即可，搜索过滤后跟随。置顶标记升级为描线图钉 SVG + 「置顶」小字（纯文字版已换代）。悬停浮现浅朱砂竖线（「可进入」记号的浅档），选中仍是实朱砂——规则写在 `.sess li.on` 之前，同特异性下选中态优先。**空态富化**：无会话时搜索框一并收起，档案盒 SVG + 「档案室还空着」+ 引导一句；搜索无果给「清空搜索 · 看全部 N 个」一键出路。
- **番茄钟撤出功能大厅宫格**（按用户要求）：`HALL_CARDS` 里 pomodoro 条目保留、加 `hidden: true`，宫格渲染 `filter(!c.hidden)`。**只藏入口不删代码**——对话里说「来一个番茄钟」照常走 manage_pomodoro → SSE `ev.pomodoro` → 对话内 `.pomo` 倒计时卡；`buildPomodoro` 面板逻辑与 `/api/pomodoro` 接口全部保留，恢复展示只需去掉 hidden 标记。
- **首屏问候动态化**：`heroGreeting()` 按本地时段换招呼（早上好/中午好/下午好/晚上好/夜深了），上方配 mono 日期戳 `TODAY · 9月22日 周二`（`#heroKicker`，与 /today 页 lead-kicker 同语言）；`startFresh` 恢复首屏时重算。
- **功能大厅图标去 emoji**：九张宫格入口全部换成与工具卡齿轮同族的 1.8px 描线 SVG（`HALL_ICONS` 常量表，settings 复用 `GEAR`；emoji 是彩色卡通，与墨色纸面跳戏）。`.hall-empty` 空态里的 emoji 保留——那是插画位不是导航位。
- **today 页**：头条倒计时字号 15→19px 加字重（「还有 N 小时 N 分开讲」是头版最重要的数字）；课格 `.tt-course` 悬停微浮起（提示有 title 详情）；正文渲染补 `text-rendering: optimizeLegibility`（与对话页对齐）。
- **knowledge 页**：条目 `.k-entry` 悬停浮起反馈；同样补 optimizeLegibility。

## 五、踩过的坑（改 UI 前必读）

1. **chatPage 是外层 TS 模板字符串**：页面 JS 里的 `\n` 必须写成 `\\n`，否则被外层转义成真实换行直接产生语法错误、整个脚本不执行（症状：页面能开但所有按钮失效，`typeof send === "undefined"`）。页面代码禁用反引号模板串与 `${`。**镜像坑（2026-09-22 修）**：前端脚本拆成独立资产 `assets/chat-app.js` 后规则反过来——里面必须写**单**反斜杠；迁移时漏改的三处残留（SSE 拆行 `split("\\n")`、设置消息 `join("\\n")`、知识摘要 `/\\s+/g`）让演示对话一律「这轮没有输出，再问一次试试」（split 拆在 JSON 转义中间导致 JSON.parse 抛错走 catch）。改 chat-app.js 后全文搜 `\\\\` 应零命中。
