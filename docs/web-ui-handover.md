...
- 布局：桌面为「左档头 + 右正文」两栏。报头下第一行就是操作钮：**「新会话」（朱砂主按钮）+「设置」**（用户要求置顶，别挪回底部）；往下是**会话档案列表**（标题 + 时间/条数，悬停出 ✕ 删除，当前会话朱砂左线高亮），底部只剩等宽页脚小字。窄屏（≤860px）左栏隐藏，顶栏右侧为「新会话 · 设置」。侧栏无恐龙徽章、无学期读数卡、无导出按钮、**无今日课程卡**（均应用户要求做减法移除；`/api/brief` 端点随之整体下线，网页不碰课表缓存，问课表直接问 Agent）。
