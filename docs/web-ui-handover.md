...
| `src/schedule-cache.ts` | 课表本地缓存（`data/schedule-cache.json`）。网页的「今日课程」卡与 TUI 启动面板共用，打开网页不会触发教务登录 |
| `src/index.ts` | Agent 创建后调用 `setChatAgent(agent)` + `startChatWeb()` 启动网页服务 |
| `src/tui/welcome.ts` | 欢迎卡片显示网页地址 |
| `tests/chat-web.test.ts` | 11 个测试：页面渲染、SSE 流式、历史累积、重置、brief 接口、marked 静态路由、多会话隔离/CRUD、工具事件详情 |
...
测试命令：`npm run typecheck && npm test`（当前 104 个测试全过）。
...
- **接口**：
  - `GET /` → 对话页面
  - `GET /api/brief` → 首屏数据（学期/周次/今日课程，读课表缓存；`historyLen` = 最近会话消息数）
...
- 布局：桌面为「左档头 + 右正文」两栏。报头下第一行就是操作钮：**「新会话」（朱砂主按钮）+「设置」**（用户要求置顶，别挪回底部）；往下依次是今日课程单（slip）、**会话档案列表**（标题 + 时间/条数，悬停出 ✕ 删除，当前会话朱砂左线高亮），底部只剩等宽页脚小字。窄屏（≤860px）左栏隐藏，顶栏右侧为「新会话 · 设置」。侧栏无恐龙徽章、无学期读数卡、无导出按钮（均应用户要求移除；`/api/brief` 的 `term/week` 字段服务端仍返回但无人消费）。
...
8. **会话 id 白名单**：路由里非法 id 返回 404 而不是兜底 `default`——历史上兜底会造成"A 删了 B 串档"，测试里专门钉了这条。
...
- `data/schedule-cache.json` 长期不更新时网页/TUI 显示的是最后已知课表；在对话里问一次课表即刷新。
