...
- 布局：桌面为「左档头 + 右正文」两栏，正文与输入区同为 800px 最大宽度，主交互字号从 16px 起。报头下一行是**「新会话」通栏朱砂主按钮**（用户要求置顶）；中间是**会话档案列表**（标题 + 时间/条数，行尾**常驻**半透明 ✕ 删除、悬停变实——早期悬停才出现，用户找不到删除入口，别再改回去）；**「设置」贴在侧栏最底部**（`margin-top:auto`，用户指定位置）。窄屏（≤960px）左栏隐藏，顶栏右侧为「新会话 · 设置」。侧栏无副标语、无页脚系统信息、无恐龙徽章、无学期读数卡、无导出按钮、**无今日课程卡**（均应用户要求做减法；`/api/brief` 端点随之整体下线，问课表直接问 Agent）。
