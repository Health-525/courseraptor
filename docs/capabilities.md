...
| 工具 | 说明 | 耗时 |
|---|---|---|
| `manage_knowledge` | 知识维护：用户分享知识（概念/公式/方法/结论/经验）即批量记录（同名自动更新不堆积）；**课程归类只认课表缓存里的真实课程**——对得上（含「高数」这类俗称）归入该课程，对不上落「未分类」，绝不编造课程名。list 可按课程/关键词筛，update 改内容与归属，delete 删除。闲聊、提问、待办不入库，拿不准时先问一句 | <1s |

网页入口：`/today` 课表页待办卡下方有「知识」卡片（最近 5 条速览 + 课程徽章 + 「查看全部」），独立页 `/knowledge` 展示全部条目（按课程分类导航、关键词搜索、确认删除），聊天页侧栏「知识库」直达。
