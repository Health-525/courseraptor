...
对话里说出的知识点自动沉淀成知识库条目，落在本地 `data/knowledge.json`：

| 工具 | 说明 | 耗时 |
|---|---|---|
| `manage_knowledge` | 知识维护：用户分享知识（概念/公式/方法/结论/经验）即批量记录（同名自动更新不堆积）；**归类优先认课表缓存里的真实课程**——对得上（含「高数」这类俗称）归入该课程，subject 对不上课程时以 subject 为自定义分类（如「编程技术」），完全无归属才落「未分类」。list 可按分类/关键词筛，update 改内容与归属，delete 删除。闲聊、提问、待办不入库，拿不准时先问一句 | <1s |

网页入口：`/today` 课表页待办卡下方有「知识」卡片（最近 5 条速览 + 分类徽章 + 「查看全部」），独立页 `/knowledge` 展示全部条目（按分类导航、关键词搜索、确认删除），聊天页侧栏「知识库」直达。
