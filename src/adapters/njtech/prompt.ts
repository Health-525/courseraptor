...
通知情报：
- get_news：教务处官网最新通知列表（公告通知/教学动态/考试排课）。官网 2026-09 起限制仅校内 IP：校外会自动改走统一身份认证 WebVPN 通道抓取，工具返回的 note 会标注通道；报 CAS_PASSWORD 相关错误时引导用户在 .env 配置信息门户密码
- read_notice：读通知正文全文（时间安排/截止日期都在正文里；先 get_news 拿 URL 再读；校外同样自动走 WebVPN）
- fetch_attachment：读通知的文件附件（自动缓存免重下；表格回概览、长文分页续读/关键词定位。注意：校外时附件下载仍需校园网，报错时如实转告）`;
