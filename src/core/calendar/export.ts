...
// 周次展开 / 节次时刻 / 星期名是学校规则（正方格式 + 学校作息表），走适配器端口
function expandWeeks(spec: string): number[] {
  return school().terms.expandWeeks(spec);
}

function periodTimeRange(periods: number[]): string | undefined {
  return school().terms.periodTimeRange(periods);
}

function weekdayName(weekday: number): string {
  return school().terms.weekdayName(weekday);
}

import type { CourseData, ExamData } from "../model";
import { listSpecialDays, specialOnDate } from "./holidays";
      ...
      for (const c of courses) {
        ...
        drafts.push({
          ...
          description: [
            ...
            `第 ${week} 周 · 调休补课（按${weekdayName(d.follows)}课表）`,
          ]
            .filter(Boolean)
            .join("\n"),
        });
