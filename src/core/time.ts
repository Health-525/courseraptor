...
import type { TermDateSource } from "./model";
import { school } from "./school";

/** 学校所在时区（适配器给出）：教务日历、周次口径全部按北京时间 */
export function schoolTimezone(): string {
  return school().info.timezone;
}
 ...
 * 周次公式与真值由学校适配器给出（terms 端口），这里只做选择与包装。
 */
function termNow(now: Date): TermNow {
  const terms = school().terms;
  for (const c of terms.candidates(now)) {
    const w = terms.weekOf(c.year, c.semester, now);
    if (w) {
      const monday = shiftDate(w.week1Monday, (w.week - 1) * 7);
      return {
        label: terms.label(c.year, c.semester),
        week: w.week,
  ...
  const first = terms.candidates(now)[0];
  const info = terms.week1MondayOf(first.year, first.semester);
  const beijingDate = calendarParts(schoolTimezone(), now);
  const today = `${beijingDate.year}-${beijingDate.month}-${beijingDate.day}`;
  return {
    label: terms.label(first.year, first.semester),
    week: null,
 ...
 */
export function getTimeReport(timezone = schoolTimezone(), now: Date = new Date()): TimeResult {
  ...
  const bj = calendarParts(schoolTimezone(), now);
