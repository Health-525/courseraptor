...
export const weatherTools = {
  ...
  get_weather: tool({
    description:
      "查询天气：实况（温度/体感/湿度/风）+ 未来若干天预报。天气码已翻成中文，并给出带伞与穿衣建议。默认查学校所在城市，用户提到别的城市就用 city 传（如「三亚」）。用户问「明天冷不冷」「要带伞吗」「周末天气」「老家天气」时直接调用，不要反问城市。",
    ...
    execute: async ({ city, days }) => {
      ...
      const r = await fetchWeather(wanted || school().info.city, days);
      if (!r.ok) {
