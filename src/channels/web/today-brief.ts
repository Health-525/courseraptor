...
// 学期周次 / 节次时刻 / 星期名是学校规则，统一走适配器端口（模块内小包装便于局部调用）
const terms = {
  weekOf: (y: number, sem: number, now?: Date) => school().terms.weekOf(y, sem, now),
  week1MondayOf: (y: number, sem: number) => school().terms.week1MondayOf(y, sem),
  expandWeeks: (spec: string) => school().terms.expandWeeks(spec),
  periodTimeRange: (periods: number[]) => school().terms.periodTimeRange(periods),
  periodTime: (period: number) => school().terms.periodTime(period),
  periodTimes: () => school().terms.periodTimes(),
  weekdayName: (n: number) => school().terms.weekdayName(n),
};

import { listKnowledge } from "../../core/knowledge";
...
function toBriefCourse(c: CourseData, nowMin: number): BriefCourse {
  ...
  return {
    ...
    ...(terms.periodTimeRange(c.periods) ? { time: terms.periodTimeRange(c.periods) } : {}),
...
function relativeDateLabel(offset: number, date: Date): string {
  ...
  return `${date.getMonth() + 1}月${date.getDate()}日 ${terms.weekdayName(weekdayOf(date))}`.trim();
}
...
function buildWeekOverview(
  ...
  for (let i = 0; i < 7; i++) {
    ...
    days.push({
      ...
      label: terms.weekdayName(weekdayOf(date)),
...
function dueLabelOf(due: Date, today: Date): string {
  ...
  return `${due.getMonth() + 1}月${due.getDate()}日 ${terms.weekdayName(weekdayOf(due))} ${hm}`.trim();
}
...
export function buildTodayBrief(now: Date = new Date(), requestedWeek?: number): TodayBrief {
  ...
  const todayIso = isoOf(today);
  const dateLabel =
    `${today.getMonth() + 1}月${today.getDate()}日 ${terms.weekdayName(weekdayOf(today))}`.trim();
  ...
  const currentWeek = terms.weekOf(schedule.year, schedule.semester, now);
