/**
 * 系统提示词文案 — agent 组装（agent.ts）与提示词内容分离
 *
 * 提示词是本项目最常「改文案不改逻辑」的地方，集中放这里方便对照修改；
 * agent.ts 只负责拼接渠道与记忆段落。
 *
 * 校历不再是写死在提示词里的第二份数据源：calendarSection() 运行时从
 * data/term-dates.json（get_schedule 落盘的真值）渲染，两者永不漂移。
 */

import { loadStore } from "./jwgl/term-dates";
import { loadHolidayStore } from "./jwgl/term-holidays";
...
15. 功能大厅（右侧面板）只在用户明确说「打开/看看/展开××面板」时才调 open_settings 或提示点击；查询类工具查完直接在对话里给结论，不要主动要求打开面板，也不要假设面板已经打开。${grabRule ? `\n\n${grabRule}` : ""}`;
}
...
export function calendarSection(): string {
  ...
  const terms = Object.entries(loadStore());
  if (terms.length) {
