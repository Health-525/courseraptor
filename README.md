...
同一个 agent，两种入口：终端里敲 `raptor` 直接对话，或浏览器打开 `http://localhost:3210` 用网页版。思考过程、工具调用、每轮问答都会归档进会话历史。工具产出的成品文件（课表/考试 .ics 日历、生成的 Word/Excel/PPT/PDF）在网页里直接点「下载」保存，不用去翻目录。
...
Agent 默认可调用 **28 个工具**（选课季 +4 抢课工具共 32 个），覆盖教务查询、通知情报、文件与数据、文档写作、时间·日历·天气、两层记忆、待办提醒、知识沉淀、QQ 接入九条线。

| 📚 教务查询 | 📰 通知情报 | 📁 文件与数据 | ✍️ 文档写作 | 🕐 时间·日历·天气 | 🧠 两层记忆 | 📝 待办与知识 | 💬 QQ 接入 |
|---|---|---|---|---|---|---|---|
| 课表/成绩/考试/学籍一句话查 | 通知正文+附件（xlsx/docx/pdf）都能读 | 千行 Excel 概览+筛选按需查，长文分页读得完 | AI 辅助写作，直接产出 Word/Excel/PPT/PDF 成品 | 日期/教学周/时区，课表考试导出 .ics 或订阅到手机，天气带穿衣建议 | 记住偏好和结论，跨会话不丢 | 待办随口记、到期提醒；知识自动沉淀按课程归类 | 官方机器人零封号，群里 @ 就能用 |
...
| `list_choosed_courses` | 本轮已选课程（选课模块维度；抢课成功后核对选没选上、退课前看现状） | ~3s |
...
随口说的安排直接变成待办，落在本地 `data/web-workspace.json`，对话、课表页、网页设置面板三个入口共用同一份数据：

| 入口 | 说明 |
|---|---|
| `manage_todos` 工具 | 对话里说出待办即批量记录（一次说几件记几件；「明天 / 下周五」这类相对日期先 `get_time` 换算，没给截止时间会先问一句）；问「我有哪些待办」用 list，完成 / 删除用 update / delete |
| 课表页待办卡片 | `/today` 本周课表下方常驻：未完成按截止排序、逾期朱砂红 ⚠、勾选完成、确认删除；与聊天页「设置 → 截止日期待办」同步 |
...
🦖   已记录 2 条待办：交高数作业（9月16日 23:59 截止）、
     实验报告（9月18日 14:00 截止），课表页和设置面板里都能看到。
```
...
对话里说出的知识点会自动沉淀：agent 判断消息里的概念、公式、方法、结论值不值得记，值得就写进本地 `data/knowledge.json`，并**只按课表里真实存在的课程归类**——对得上就归入该课程（「高数」这类俗称也能对上），对不上落「未分类」，绝不编造课程名：

| 入口 | 说明 |
|---|---|
| `manage_knowledge` 工具 | 对话里分享知识即记录（一次说几条记几条，同名知识自动更新不堆积）；问「我记过哪些知识」用 list（可按课程/关键词筛），改内容或归属用 update，删用 delete。闲聊、提问、待办不会误入库，拿不准时 agent 会先问一句 |
| 课表页知识卡片 | `/today` 待办卡下方常驻：最近 5 条速览（标题 + 课程徽章 + 日期 + 内容两行摘录）、总数统计、「查看全部」链到独立页 |
| 独立知识库页 | `/knowledge`：全部条目按课程分类导航（带计数）、关键词搜索、确认删除；聊天页侧栏「知识库」直达 |
...
选课季完整作战链：**查状态 → 搜课比班 → 盯余量 → 自动抢 → 核对/退课**。抢课/退课是**真实选课提交操作**，平时隐藏；选课季在 `.env` 设 `RAPTOR_ENABLE_GRAB=1` 重启即启用：

```text
你   通识选修开抢了：创新创业类和人文类各帮我抢一门网课，
     备选「创新创业基础」「大学生创业导论」「中国哲学智慧」
🦖   分组计划：创新创业类 [创新创业基础 → 大学生创业导论]、
     人文类 [中国哲学智慧]，每类抢到一门即停。
     抢课执行中（最长 600 秒，每轮约 2.5 秒，满员自动切备选）……
🦖   🎉 [创新创业类] 抢到：创新创业基础（网课），该类收手
     [人文类] 中国哲学智慧持续满员，备选已用尽
     总结：抢到 1/2 类（37 轮监控）。已选清单里核对到了，继续盯人文类余量吗？
```

| 工具 | 说明 |
|---|---|
| `watch_courses` | 盯课：限时轮询目标课程余量变化（默认 60 秒，间隔约 3 秒带随机抖动），只观察不提交 |
| `grab_course` | 抢课（单目标）：一出现余量立即自动提交选课，成功即停（默认 120 秒，提交重试上限 10 次） |
| `grab_plan` | 分类抢课计划：按类别分组抢课，每类抢到一门即停、**绝不重复抢同类学分**；候选满员 3 轮或未出现 5 轮自动切换备选（默认 600 秒） |
| `drop_course` | 退课：用户点名才退、绝不批量；多门命中或教学班无法唯一定位时**拒绝提交**，返回明细让用户指认 |

配合常驻的查询工具组成完整闭环：`check_selection_status`（选课是否开放、接口是否被防爬拦截）、`search_courses` / `search_classes`（搜课查余量、同门课各教学班对比）、`list_choosed_courses`（本轮已选清单——抢完核对是否选上、退课前看现状）。

**抢课引擎（2026-08-27 首轮正选实战校准）**：

- **会话自愈**：抢课途中登录态失效自动重登（线性退避、上限 10 秒），线路抖动连续 5 次异常才放弃；监控期间新轮次上线（如 12:00 通识选修轮开新 tab）自动限频刷新会话重解析
- **协议全链路**：正方 V9 加密串（`xkkz_xh`）、多轮次 tab 凭证、Display 页隐藏字段均已攻克，查询与提交严格同源；主提交接口异常时自动切换备用接口
- **克制设计**：轮询间隔带随机抖动、全局令牌桶限速（`RAPTOR_MAX_RPS` 仅允许下调，尊重教务系统承载），抢到即停不重复占名额

已选课程查询、退课、备用提交接口的路径与参数移植自同校实测脚本 [njtech_grabber](https://github.com/Health-525/njtech_grabber)，待下一轮选课开放实测复核。
...
- **已接入 28 个工具（+4 抢课季工具，共 32）**：课表 / 成绩 GPA / 考试 / 学籍 / 已选教学班 / 可重修 / 实验成绩 / 选课查询四件套 / 教务处通知三件套 / 文件与数据四件套（本地文件读取、Excel 筛选、沙箱计算、附件缓存管理）/ 文档写作两件套（Word·Excel·PPT·PDF 生成与跨格式转换）/ 放假调休落盘 / 时间（日期/教学周/时区，模型回答任何时间问题前的必查项）/ 整学期 .ics 日历导出与发布（课表+考试，含调休与假期处理；发布到 Gitee（国内直连）或 GitHub 后手机日历订阅链接自动更新）/ 天气 / 记忆 / 待办（对话记录 + 课表页展示 + 到期提前一周自动提醒）/ 知识库（对话自动判断记录 + 按课表课程归类 + /knowledge 页检索）
...
└── src/
    ...
    ├── knowledge.ts    #   知识库（对话沉淀的知识条目：课表课程自动归类 + 未分类兜底 + 本地存储）
    ...
    │   └── types.ts
    └── tools/          #   agent 工具（默认 28 个，抢课季 +4）+ 会话缓存管理
```
