...
| `drop_course` | 退课：用户点名才退，多门命中/教学班定位不明时拒绝提交并让用户指认 |

选课协议已实战校准（2026-08-27 首轮正选实测）：加密串（`xkkz_xh`）、多轮次 tab、Display 页隐藏字段、平铺数据结构均已攻克。已选课程查询、退课、备用提交接口的路径与参数移植自同校实测脚本 [njtech_grabber](https://github.com/Health-525/njtech_grabber)，待下一轮选课实测复核。
