...
| 工具 | 说明 | 耗时 |
|---|---|---|
| `check_selection_status` | 选课模块状态：是否开放（iskxk）、接口是否被防爬拦截 | ~3s |
...
| `grab_plan` | 分类抢课计划：每类抢到一门即停（不重复抢同类学分），满员自动切备选 | 指定时长 |
...
| `get_student_info` | 学籍个人信息（学院/专业/班级/年级等档案字段） | ~3s |
...
- **已接入 16 个工具**：课表 / 选课全套 / 成绩 GPA / 考试 / 学籍个人信息 / 已选教学班 / 可重修课程 / 实验成绩 / 教务处通知三件套 / 记忆
